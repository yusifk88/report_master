...
while ($row = mysqli_fetch_object($search)) {
    ...
    <div class="col-md-3 col-sm-3 col-3">
        <img class="img-responsive img-fluid" src="<?= file_exists($row->photo)?$row->photo:'objts/dpic/photo.jpg';?>" />
        ...
        <div class="row">
            ...
            <div class="col-lg-4 col-md-4 col-sm-4 col-12 text-muted">
                ...
                <div class="col-md-2 col-2">
                    <button data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="staction<?=$row->id?>" class="btn btn-sm text-center align-text-middle bg-transparent dropdown-toggle"><i class="fa fa-ellipsis-v text-muted"></i></button>
                    <ul class="dropdown-menu animated zoomIn" aria-labelledby="staction<?=$row->id?>" >
                        <li class="dropdown-item text-muted" style="cursor:pointer;"><a onclick="upstud(<?=$row->id;?>)">Edit Info.</a></li>
                        <li class="dropdown-item text-muted" style="cursor:pointer;"><a onclick="printstud(<?=$row->id;?>)">Print Profile</a></li>
                        <li class="dropdown-item text-muted" style="cursor:pointer;"><a onclick="Comnt(<?=$row->id;?>)">Add Comment</a></li>
                        <li class="dropdown-item text-muted" style="cursor:pointer;"><a onclick="printrep(<?=$row->id;?>)">Print Reports</a></li>
                        <li class="dropdown-item text-muted" style="cursor:pointer;"><a onclick="printtransc(<?=$row->id;?>)">Print Transcript</a></li>
                        <li class="dropdown-item text-muted" style="cursor:pointer;"><a onclick="resetlogin(<?=$row->id;?>)">Reset Password</a></li>
                        <li class="dropdown-item text-danger" style="cursor:pointer;"><a onclick="withdraw(<?=$row->id;?>)">Withdraw</a></li>
                        <li class="dropdown-item text-danger" style="cursor:pointer;"><a onclick="delstud(<?=$row->id;?>)">Delete</a></li>
                    </ul>
                </div>
