...
$subjt = $_GET['subjt'];
$per_page = 150;
$page_query = mysqli_query($cf->con,"select count(records.id) as recnum from records,stuinfo,classes,subjects where records.subjt = subjects.id and records.cls = classes.id and records.stid = stuinfo.id and records.term = '$term' and records.acyear='$ayear' and records.subjt = '$subjt' and records.cls = '$cls'");
$n = mysqli_fetch_object($page_query);
$pages = ceil($n->recnum/$per_page);
$page = ( isset($_GET['page'])) ? (int) $_GET['page']:1;
$start = ($page-1) * $per_page;

$recs = mysqli_query($cf->con,"select records.subjt,records.id,stuinfo.id as stid,stuinfo.fname,stuinfo.lname,stuinfo.oname,classes.classname,records.term,records.acyear,subjects.subjdesc,ta1,ta2,ta3,ta4,hw1,hw2,hw3,hw4,pw1,pw2,subtotl,cvsubtotl,exam,cvexam,totlscore,post ,((subtotl*$sch->clscore_ratio)/$sch->sba+(exam*($sch->exam_ratio/100))) as tscore from records,stuinfo,classes,subjects where records.subjt = subjects.id and records.cls = classes.id and records.stid = stuinfo.id and records.term = '$term' and records.acyear='$ayear' and records.subjt = '$subjt' and records.cls = '$cls' order by tscore DESC ");
    $sch = new school();
...
<div class="table-responsive">    
    ...
    <thead>
        <tr style="background-color: deepskyblue; color:#fff;">
            ...
            <th colspan="4" style="text-align: center; border: 1px solid deepskyblue; border-right: 1px solid #fff;">
                Class Test
            </th>
...
<!--            </th>-->
    ...
    <?php
        ...
        <tr id="row_<?=$recrow['id']?>">
            <td><?=$a;?></td>
            <td><a class="btn btn-link btn-default" onclick="printstud(<?=$recrow['stid']?>); return false;"><?= $recrow['fname']." ".$recrow['lname']." ".$recrow['oname'];?></a></td>
       ...
       fullProg();
        $.get("getassessupdate.php?id="+id,function(data){
            ...
            BootstrapDialog.show({
                ...
                message:data,
                size:"size-wide",
                buttons:[{label:"CANCEL",cssClass:'btn-bad waves-effect waves-button',action:function(d){d.close()}},{label:"UPDATE",cssClass:"btn-good waves-effect waves-button",action:function(d){
