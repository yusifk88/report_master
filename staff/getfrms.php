                ...
                <table class="table table-condensed table-hover table-striped table-bordered" id="assess-table">
                    ...
                    while($recrow = mysqli_fetch_assoc($recs)){
                        ...
                        <tr id="frm_row<?=$recrow['id']?>">
                            ...
                            <td style="text-align: center; vertical-align: middle;"><i data-toggle='tooltip' onclick="updatefrm(<?=$recrow['id']?>,<?=$recrow['stid']?>,<?=$recrow['term']?>,'<?=$recrow['ayear'];?>')" title="Make changes to this record" style="color: deepskyblue; cursor: pointer;" class="fa fa-edit waves-effect waves-circle"></i></td>
                            <td style="text-align: center; vertical-align: middle;"><i data-toggle='tooltip' onclick="delfrm(<?=$recrow['id'];?>)" title="Delete this record permanently" style="color: red; cursor: pointer;" class="fa fa-remove waves-effect waves-circle"></i></td>
                        </tr>
                        <?php
                        $a++;
                    }
...
if($pages > 1){
    ...
    if(($page < $pages)){								?>
        ...
        <li><a style="cursor:pointer;" onclick="assess_page(<?= $pages;?>)">Last Page &GreaterGreater;&GreaterGreater; </a></li>
        <?php
    }
    ?>
        ...
        $.get("getfrmupdate.php?id="+id+"&term="+term+"&ayear="+ayear+"&stid="+stid,function(data){
            ...
            BootstrapDialog.show({
                ...
                buttons:[{label:"Update",cssClass:"btn-good waves-effect waves-button",action:function(d){
                   fullProg();
                    $.get("upfrm.php?id=" + $("#upfrmid").val() + "&attndance=" + $("#upfrmattndance").val() + "&attnded=" + $("#upfrmattnded").val() + "&cnduct=" + $("#upfrmcnduct").val() + "&interest=" + $("#upfrminterest").val() + "&remark=" + $("#upfrmremark").val()+"&ayear="+$("#frm-ayear").val()+"&term="+$("#frm-term").val(), function (data) {$("#frm_class").change();d.close();});
